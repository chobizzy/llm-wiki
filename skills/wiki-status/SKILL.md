...
1. **Resolve config** — follow the Config Resolution Protocol in `llm-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.llm-wiki/config` → prompt setup). This gives `OBSIDIAN_VAULT_PATH`, `OBSIDIAN_SOURCES_DIR`, `CLAUDE_HISTORY_PATH`, and `HERMES_HOME`.
...
### Hermes History (from `HERMES_HOME`)
```
Glob: $HERMES_HOME/**/*.md and session artifacts — see `hermes-history-ingest` for the exact layout
Record: path, size, modification time
```
Skip this section if `HERMES_HOME` is unset and `~/.hermes` doesn't exist.
...
```bash
llm-wiki graph-analyse "$OBSIDIAN_VAULT_PATH" --pretty
```
...
**Fallback** (if `llm-wiki` is not installed): glob all `.md` pages, extract every `[[wikilink]]`, and build `incoming`, `outgoing`, and `tags` maps manually.
...
- The actual ingest work is done by the ingest skills (`wiki-ingest`, `claude-history-ingest`, `hermes-history-ingest`)
