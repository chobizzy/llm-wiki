...
1. **Resolve config** — follow the Config Resolution Protocol in `llm-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.llm-wiki/config` → prompt setup). For cross-project queries without `@name`, prefer `~/.llm-wiki/config` when present, even if it is a symlink to the vault `.env`. This gives `OBSIDIAN_VAULT_PATH` and any QMD variables. Works from any project directory.
...
```bash
llm-wiki graph-query "$OBSIDIAN_VAULT_PATH" "<question>" --pretty
```
...
**Fallback** (if `llm-wiki` is not installed): proceed with Step 1 as normal using grep and index.md.
