...
1. **Resolve config** — follow the Config Resolution Protocol in `llm-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.llm-wiki/config` → prompt setup). This gives `OBSIDIAN_VAULT_PATH` and `LLM_WIKI_REPO`.
...
Check that `$LLM_WIKI_REPO/scripts/daily-update.sh` exists and is executable. If not, point the user to it.

**Step 2: Install launchd plist**

```bash
# Replace placeholder in plist
sed "s|LLM_WIKI_REPO|$LLM_WIKI_REPO|g" \
  "$LLM_WIKI_REPO/scripts/com.llm-wiki.daily-update.plist" \
  > "$HOME/Library/LaunchAgents/com.llm-wiki.daily-update.plist"

# Load it
launchctl load "$HOME/Library/LaunchAgents/com.llm-wiki.daily-update.plist"
```
...
```bash
echo "" >> "$RC_FILE"
echo "# llm-wiki terminal notification" >> "$RC_FILE"
echo "source $LLM_WIKI_REPO/scripts/wiki-notify.sh" >> "$RC_FILE"
```

For Fish shell, source syntax is different — provide the manual instruction:
```fish
# Add to ~/.config/fish/config.fish:
bass source $LLM_WIKI_REPO/scripts/wiki-notify.sh
...
```bash
bash "$LLM_WIKI_REPO/scripts/daily-update.sh"
```
...
- State is stored in `~/.llm-wiki/state/<vault-id>/` — supports multiple vaults independently
- They can run `/daily-update` anytime to force a sync
- Logs go to `/tmp/llm-wiki-daily.log`

## QMD Refresh After Vault Writes

**GUARD: If `$QMD_WIKI_COLLECTION` is unset — the default — skip this step.** QMD is a
search index, not the source of truth.

When it is set, **read `llm-wiki/references/qmd.md` and follow the *Refresh* section**
after this skill has written vault markdown. If refresh fails, do not roll back the
vault changes; report the QMD status separately.
