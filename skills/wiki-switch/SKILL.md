...
description: >
  ...
  The skill manages named config files at ~/.llm-wiki/config.NAME and activates one by
  symlinking it to ~/.llm-wiki/config.
---

# Wiki Switch — Manage Multiple Vault Profiles

Each vault is a complete config file at `~/.llm-wiki/config.<name>`. The active vault is
whichever file `~/.llm-wiki/config` symlinks to. Switching vaults means re-pointing that symlink.
...
in `llm-wiki/SKILL.md`, not by this skill — it resolves `~/.llm-wiki/config.<name>` for that one
...
1. Verify `~/.llm-wiki/config.<name>` exists. If not, tell the user the vault doesn't exist and list what's available (run **List**).
2. Run:
   ```bash
   ln -sf ~/.llm-wiki/config.<name> ~/.llm-wiki/config
   ```
...
1. Find all files matching `~/.llm-wiki/config.*` (exclude `config` itself — that's the symlink).
2. Resolve the current symlink target: `readlink ~/.llm-wiki/config`
...
- If a name is given, read `~/.llm-wiki/config.<name>`.
- If no name given, read `~/.llm-wiki/config` (the active vault).
...
1. Check `~/.llm-wiki/config.<name>` doesn't already exist. Abort if it does.
2. Copy the active config:
   ```bash
   cp ~/.llm-wiki/config ~/.llm-wiki/config.<name>
   ```
