...
1. **Resolve config** — follow the Config Resolution Protocol in `llm-wiki/SKILL.md` (inline `@name` override → walk up CWD for `.env` → `~/.llm-wiki/config` → prompt setup). This gives `OBSIDIAN_VAULT_PATH` and optional QMD settings such as `QMD_WIKI_COLLECTION`
...
3. `hermes-history-ingest` — to reprocess Hermes history
...
**GUARD: If `$QMD_WIKI_COLLECTION` is unset — the default — skip this step.** QMD is a search index, not the source of truth.

When it is set, **read `llm-wiki/references/qmd.md` and follow the *Refresh* section** once the rebuilt vault files are on disk. A rebuild replaces page content wholesale, so expect the embedding pass to be needed. If refresh fails, keep the rebuilt vault and report the QMD status separately.
