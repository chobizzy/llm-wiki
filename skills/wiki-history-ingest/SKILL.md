...
description: >
  ...
  "/wiki-history-ingest claude", "/wiki-history-ingest hermes", or asks to ingest agent history
  without naming the underlying skill. This router dispatches to the specialized history skill.
---
...
1. If the user explicitly says `claude` or `hermes`, route directly.
2. If the user provides a path/source:
   - `~/.claude` or Claude memory/session JSONL artifacts -> `claude-history-ingest`
   - `~/.hermes`, `HERMES_HOME`, or Hermes memories/session artifacts -> `hermes-history-ingest`
3. If ambiguous, ask one short clarification:
   - "Should I ingest `claude` or `hermes` history?"
4. If the user names an agent with no matching skill installed (e.g. `copilot`, `codex`,
   `openclaw`, `pi`), say the corresponding history-ingest skill is not installed and fall
   back to `wiki-ingest` on the raw export files instead.
