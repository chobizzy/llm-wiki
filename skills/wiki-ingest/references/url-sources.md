...
4. **No project context** — if the current directory IS the llm-wiki repo itself, or if detection produces a name that matches the wiki vault directory, treat it as "no project context" and fall back to `misc/`.
